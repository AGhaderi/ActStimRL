        ...
        for subName in subList:
            for session in [1,2]:
                for reverse in [14, 21]:
                    for block in ['Stim', 'Act']:
                        # yellow and pushed correct for each condition
                        rawBehAll1 = rawBehAll[(rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']==block) &(rawBehAll['reverse']==reverse)]                     
                        # list of win amount
                        winAmt = np.array(rawBehAll1['winAmtLeft'])
                        np.random.shuffle(winAmt)
                        winAmtPushable = winAmt
                        winAmtPullable = 100 - winAmt
                        # random numder for yellow an blue
                        rng = np.concatenate([np.ones(int(np.ceil(len(winAmt)/2))), np.zeros(int(np.floor(len(winAmt)/2)))])
                        np.random.shuffle(rng)
                        winAmtYellow = rng*winAmtPushable + (1-rng)*winAmtPullable
                        winAmtBlue = 100 - winAmtYellow
                                                
                        # substitude yellow and pushed amounts
                        rawBehAll.loc[((rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']==block) &(rawBehAll['reverse']==reverse)), 'winAmtPushable'] = winAmtPushable
                        rawBehAll.loc[((rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']==block) &(rawBehAll['reverse']==reverse)), 'winAmtPullable'] = winAmtPullable
                        rawBehAll.loc[((rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']==block) &(rawBehAll['reverse']==reverse)), 'winAmtYellow'] = winAmtYellow
                        rawBehAll.loc[((rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']==block) &(rawBehAll['reverse']==reverse)), 'winAmtBlue'] = winAmtBlue
