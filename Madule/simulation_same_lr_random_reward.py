...
def set_true_all_parts(alpha_mu_arg, alpha_sd_arg,
    ...
    try:
        ...
        # switch pushed and yellow correct one another
        for subName in subList:
            for session in [1,2]:
                for reverse, nphase in zip([14, 21], [3, 2]):
                    for i in range(1, 1 + nphase):

                        # yellow and pushed correct for Color condition
                        yellowCorrect_stim = rawBehAll[(rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']=='Stim') &(rawBehAll['reverse']==reverse)& (rawBehAll['phase']==f'phase{i}')]['yellowCorrect'].to_numpy()
                        np.random.shuffle(yellowCorrect_stim)
                        yellowOnLeftSide_stim = rawBehAll[(rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']=='Stim') &(rawBehAll['reverse']==reverse)& (rawBehAll['phase']==f'phase{i}')]['yellowOnLeftSide'].to_numpy()
                        leftCanBePushed_stim = rawBehAll[(rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']=='Stim') &(rawBehAll['reverse']==reverse)& (rawBehAll['phase']==f'phase{i}')]['leftCanBePushed'].to_numpy()
                        indicator_in_stim = yellowOnLeftSide_stim==leftCanBePushed_stim
                        rawBehAll.loc[((rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']=='Stim') &(rawBehAll['reverse']==reverse)& (rawBehAll['phase']==f'phase{i}')), 'yellowCorrect'] = yellowCorrect_stim
                        rawBehAll.loc[((rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']=='Stim') &(rawBehAll['reverse']==reverse)& (rawBehAll['phase']==f'phase{i}')), 'pushCorrect'] = (indicator_in_stim==yellowCorrect_stim).astype(int)

                        # yellow and pushed correct for Action condition
                        pushCorrect_act = rawBehAll[(rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']=='Act') &(rawBehAll['reverse']==reverse)& (rawBehAll['phase']==f'phase{i}')]['pushCorrect'].to_numpy()
                        np.random.shuffle(pushCorrect_act)
                        yellowOnLeftSide_act = rawBehAll[(rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']=='Act') &(rawBehAll['reverse']==reverse)& (rawBehAll['phase']==f'phase{i}')]['yellowOnLeftSide'].to_numpy()
                        leftCanBePushed_act = rawBehAll[(rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']=='Act') &(rawBehAll['reverse']==reverse)& (rawBehAll['phase']==f'phase{i}')]['leftCanBePushed'].to_numpy()
                        indicator_in_act = yellowOnLeftSide_act==leftCanBePushed_act
                        rawBehAll.loc[((rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']=='Act') &(rawBehAll['reverse']==reverse)& (rawBehAll['phase']==f'phase{i}')), 'pushCorrect'] = pushCorrect_act
                        rawBehAll.loc[((rawBehAll['sub_ID']==subName)& (rawBehAll['session']==session) &(rawBehAll['block']=='Act') &(rawBehAll['reverse']==reverse)& (rawBehAll['phase']==f'phase{i}')), 'yellowCorrect'] = (indicator_in_act==pushCorrect_act).astype(int)
