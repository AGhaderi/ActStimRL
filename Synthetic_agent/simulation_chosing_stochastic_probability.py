            ...
            for cond in ['Stim', 'Act']:
                if cond=='Act':
                    ...
                    if actData['reverse'].unique()==21:
                        ...
                        # choose option stochastically
                        stoch_ = np.random.binomial(n=1, p=.75, size=len(actDataPhase1)) 
                        if round(propPhase1) == 1:
                            choice_stoch_ = stoch_
                        else:
                            choice_stoch_ = 1 - stoch_
                        # Chossing option with higher probability reward, pushed
                        rawBehAll.loc[(rawBehAll['session']==sess) & (rawBehAll['run']==run) & (rawBehAll['block']==cond) & (rawBehAll['sub_ID']==subName)& (rawBehAll['phase']=='phase1'), 'pushed_agent'] = choice_stoch_
                        # Phase 2
                        actDataPhase2 = actData[actData['phase']=='phase2']
                        propPhase2 = actDataPhase2['pushCorrect'].mean()
                        # choose option stochastically
                        stoch_ = np.random.binomial(n=1, p=.75, size=len(actDataPhase2)) 
                        if round(propPhase2) == 1:
                            choice_stoch_ = stoch_
                        else:
                            choice_stoch_ = 1 - stoch_
                        # Chossing option with higher probability reward
                        rawBehAll.loc[(rawBehAll['session']==sess) & (rawBehAll['run']==run) & (rawBehAll['block']==cond) & (rawBehAll['sub_ID']==subName)& (rawBehAll['phase']=='phase2'), 'pushed_agent'] = choice_stoch_
                    # if the reversal point is 14
                        ...
                        # choose option stochastically
                        stoch_ = np.random.binomial(n=1, p=.75, size=len(actDataPhase3)) 
                        if round(propPhase3) == 1:
                            choice_stoch_ = stoch_
                        else:
                            choice_stoch_ = 1 - stoch_
                        # Chossing option with higher probability reward
                        rawBehAll.loc[(rawBehAll['session']==sess) & (rawBehAll['run']==run) & (rawBehAll['block']==cond) & (rawBehAll['sub_ID']==subName)& (rawBehAll['phase']=='phase3'), 'pushed_agent'] = choice_stoch_
                    ...
                    if actData['reverse'].unique()==21:
                        ...
                        if round(propPhase1) == 1:
                            ...
                            choice_stoch_ = 1 - stoch_
                        # Chossing option with higher probability reward, pushed
                        rawBehAll.loc[(rawBehAll['session']==sess) & (rawBehAll['run']==run) & (rawBehAll['block']==cond) & (rawBehAll['sub_ID']==subName)& (rawBehAll['phase']=='phase1'), 'yellowChosen_agent'] = choice_stoch_
                        # Phase 2
                        actDataPhase2 = actData[actData['phase']=='phase2']
                        ...
                        if round(propPhase2) == 1:
                            ...
                            choice_stoch_ = 1 - stoch_
                        # Chossing option with higher probability reward
                        rawBehAll.loc[(rawBehAll['session']==sess) & (rawBehAll['run']==run) & (rawBehAll['block']==cond) & (rawBehAll['sub_ID']==subName)& (rawBehAll['phase']=='phase2'), 'yellowChosen_agent'] = choice_stoch_
                    # if the reversal point is 14
                    elif actData['reverse'].unique()==14:
                        # Phase 1
                        actDataPhase1 = actData[actData['phase']=='phase1']
                        ...
                        if round(propPhase3) == 1:
                            ...
                            choice_stoch_ = 1 - stoch_
                        # Chossing option with higher probability reward
                        rawBehAll.loc[(rawBehAll['session']==sess) & (rawBehAll['run']==run) & (rawBehAll['block']==cond) & (rawBehAll['sub_ID']==subName)& (rawBehAll['phase']=='phase3'), 'yellowChosen_agent'] = choice_stoch_
...
rawBehAll.to_csv('/mrhome/amingk/Documents/7TPD/ActStimRL/Synthetic_agent/simulation_chosing_stochastic_probability.csv', index=False)
