...
parameters {
    ...
    array[nMeds] real hier_weight_mu_Clr;          // Mean Hierarchical Wieghtening of Color Learning Value and medication effect
    ...
    array[nParts, nMeds] real z_weight_Clr;          // Wieghtening of Color Learning Value and medication effect
...
transformed parameters {
    ...
    array[nParts, nMeds] real<lower=0, upper=1> transfer_weight_Clr;  // Wieghtening of Action Learning Value and medication effect
    ...
    array[nMeds] real<lower=0, upper=1> transfer_hier_weight_mu_Clr;  // Hierarchical Wieghtening of Color vluea learning and medication effect
   ...
   for (i in 1:N) {
        ...
        // Relative contribution of Color Value Learning
        if (condition[i]==2){
            EV_push_yell = transfer_weight_Clr[participant[i], medication[i]]*EV_push + (1 - transfer_weight_Clr[participant[i], medication[i]])*EV_yell;
            EV_push_blue = transfer_weight_Clr[participant[i], medication[i]]*EV_push + (1 - transfer_weight_Clr[participant[i], medication[i]])*EV_blue;
            EV_pull_yell = transfer_weight_Clr[participant[i], medication[i]]*EV_pull + (1 - transfer_weight_Clr[participant[i], medication[i]])*EV_yell;
            EV_pull_blue = transfer_weight_Clr[participant[i], medication[i]]*EV_pull + (1 - transfer_weight_Clr[participant[i], medication[i]])*EV_blue;
        }
