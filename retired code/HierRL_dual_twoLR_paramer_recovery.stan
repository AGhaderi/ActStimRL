...
parameters {
    ...
    array[nMeds_nSes] real hier_alphaAct_pos_mu;    // Mean Hierarchical Positive Learning rate for action Learning Value and medication_session effect
    array[nMeds_nSes] real hier_alphaAct_neg_mu;    // Mean Hierarchical Negative Learning rate for action Learning Value and medication_session effect
    array[nMeds_nSes] real hier_alphaClr_pos_mu;    // Mean Hierarchical Positive Learning rate for color Learning Value and medication_session effect
    array[nMeds_nSes] real hier_alphaClr_neg_mu;    // Mean Hierarchical Negative Learning rate for color Learning Value and medication_session effect
    ...
    array[nMeds_nSes] real hier_sensitivity_mu;    // Mean Hierarchical snesitivity, With a higher sensitivity value θ, choices are more sensitive to value differences

    /* Hierarchical sd parameter*/                               
    array[nMeds_nSes] real<lower=0> hier_alpha_sd;      // Between-participant variability Learning rate for Learning Value
    array[nMeds_nSes] real<lower=0> hier_weight_sd;     // Between-participant variability Wieghtening of Action Learning Value against to Color Learnig Value
    array[nMeds_nSes] real<lower=0> hier_sensitivity_sd;   // Between-participant variability sensitivity

    /* participant-level main paameter*/
    array[nParts, nMeds_nSes] real z_alphaAct_pos;   // Positive Learning rate for Action Learning Value
    array[nParts, nMeds_nSes] real z_alphaAct_neg;   // Negative Learning rate for Action Learning Value
    array[nParts, nMeds_nSes] real z_alphaClr_pos;   // Positive Learning rate for Color Learning Value
    array[nParts, nMeds_nSes] real z_alphaClr_neg;   // Negative Learning rate for Color Learning Value
    array[nParts, nMeds_nSes, nConds] real z_weight;  // Wieghtening of Action Learning Value against to Learnig Value
    array[nParts, nMeds_nSes] real z_sensitivity;         // With a higher sensitivity value θ, choices are more sensitive to value differences
...
transformed parameters {
    ...
    array[nParts, nMeds_nSes] real<lower=0, upper=1> transfer_alphaAct_pos;   // Poistive Learning rate for Action Learning Value
    array[nParts, nMeds_nSes] real<lower=0, upper=1> transfer_alphaAct_neg;   // Negative Learning rate for Action Learning Value
    array[nParts, nMeds_nSes] real<lower=0, upper=1> transfer_alphaClr_pos;   // Positive Learning rate for Color Learning Value
    array[nParts, nMeds_nSes] real<lower=0, upper=1> transfer_alphaClr_neg;   // Negative Learning rate for Color Learning Value
    ...
    array[nParts, nMeds_nSes] real<lower=0> transfer_sensitivity;         // With a higher sensitivity value θ, choices are more sensitive to value differences
    
    /* Transfer Hierarchical parameters just for output*/
    array[nMeds_nSes] real<lower=0, upper=1> transfer_hier_alphaAct_pos_mu;   // Hierarchical Positive Learning rate for Action Learning Value
    array[nMeds_nSes] real<lower=0, upper=1> transfer_hier_alphaAct_neg_mu;   // Hierarchical Negative Learning rate for Action Learning Value
    array[nMeds_nSes] real<lower=0, upper=1> transfer_hier_alphaClr_pos_mu;   // Hierarchical Positive Learning rate for Color Learning Value
    array[nMeds_nSes] real<lower=0, upper=1> transfer_hier_alphaClr_neg_mu;   // Hierarchical Negative  Learning rate for Color Learning Value
    ...
    array[nMeds_nSes] real<lower=0> transfer_hier_sensitivity_mu;         // Hierarchical snesitivity, With a higher sensitivity value θ, choices are more sensitive to value differences
    ...
    for (p in 1:nParts) {
        for (g in 1:nMeds_nSes){
            for (c in 1:nConds){
                transfer_weight[p, g, c] = Phi(hier_weight_mu[g, c] + z_weight[p, g, c]*hier_weight_sd[g]);
            }
                transfer_alphaAct_pos[p, g] = Phi(hier_alphaAct_pos_mu[g] + z_alphaAct_pos[p, g]*hier_alpha_sd[g]);
                transfer_alphaAct_neg[p, g] = Phi(hier_alphaAct_neg_mu[g] + z_alphaAct_neg[p, g]*hier_alpha_sd[g]);
                transfer_alphaClr_pos[p, g] = Phi(hier_alphaClr_pos_mu[g] + z_alphaClr_pos[p, g]*hier_alpha_sd[g]);
                transfer_alphaClr_neg[p, g] = Phi(hier_alphaClr_neg_mu[g] + z_alphaClr_neg[p, g]*hier_alpha_sd[g]);
                transfer_sensitivity[p, g] = log(1 + exp(hier_sensitivity_mu[g] + z_sensitivity[p,g]*hier_sensitivity_sd[g]));
        }
    }
   ...
   for (i in 1:N) {
        ...
        if (pushed[i] == 1){
            ...
            if((rewarded[i] - p_push)>=0 ){ 
                p_push = p_push + transfer_alphaAct_pos[participant[i], medication_session[i]]*(rewarded[i] - p_push);
            } 
            // negative RPE
            else{
                p_push = p_push + transfer_alphaAct_neg[participant[i], medication_session[i]]*(rewarded[i] - p_push); 
            }
        }
        else{
            // positive RPE
            if((rewarded[i] - p_pull)>=0){ 
                p_pull = p_pull + transfer_alphaAct_pos[participant[i], medication_session[i]]*(rewarded[i] - p_pull);
            } 
            // negative RPE
            else{
                p_pull = p_pull + transfer_alphaAct_neg[participant[i], medication_session[i]]*(rewarded[i] - p_pull);
            }
        }   
        ...
        if (yellowChosen[i] == 1){
            ...
            if((rewarded[i] - p_yell)>=0){ 
                p_yell = p_yell + transfer_alphaClr_pos[participant[i], medication_session[i]]*(rewarded[i] - p_yell);
            } 
            // negative RPE
            else{
                p_yell = p_yell + transfer_alphaClr_neg[participant[i], medication_session[i]]*(rewarded[i] - p_yell);
            }
        }    
        else{
            // positive RPE
            if((rewarded[i] - p_blue)>=0){ 
                p_blue = p_blue + transfer_alphaClr_pos[participant[i], medication_session[i]]*(rewarded[i] - p_blue);
            } 
            // negative RPE
            else{
                p_blue = p_blue + transfer_alphaClr_neg[participant[i], medication_session[i]]*(rewarded[i] - p_blue);
            }
        }
    }   
}
model { 
    ...
    for (g in 1:nMeds_nSes){
        hier_alphaAct_pos_mu[g] ~ normal(0,1);
        hier_alphaAct_neg_mu[g] ~ normal(0,1);
        hier_alphaClr_pos_mu[g] ~ normal(0,1);
        hier_alphaClr_neg_mu[g] ~ normal(0,1);
        hier_sensitivity_mu[g] ~ normal(0,1); 

        for (c in 1:nConds){
            hier_weight_mu[g,c] ~ normal(0,1);
        }

        hier_alpha_sd[g] ~ normal(0,1);  
        hier_weight_sd[g] ~ normal(0,1); 
        hier_sensitivity_sd[g] ~ normal(0,1);
    }
    ...
    for (p in 1:nParts) {
        for (g in 1:nMeds_nSes){
            z_alphaAct_pos[p, g] ~ normal(0,1);
            z_alphaAct_neg[p, g] ~ normal(0,1);
            z_alphaClr_pos[p, g] ~ normal(0,1);
            z_alphaClr_neg[p, g] ~ normal(0,1);
