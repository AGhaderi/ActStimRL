...
parameters {
    ...
    array[nParts, nConds] real<lower=0, upper=1> theta;          // mixing proportions
}
...
model { 
    ...
    for (i in 1:N) { 
        vector[2] lps;
        lps[1] = log(theta[participant[i], condition[i]]) + bernoulli_lpmf(pushed[i] | soft_max_EV1[i]);  // first term of mixture 
        lps[2] = log1m(theta[participant[i], condition[i]]) + bernoulli_lpmf(pushed[i] | soft_max_EV2[i]);  // second term of mixture
