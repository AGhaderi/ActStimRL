...
parameters {
    ...
    /* participant-level main parameters */
...
transformed parameters {
    /* Trnasformed hierarchical parameters */
    ...
    /* Transformation of idividual.level parameters */
 	  for (p in 1:nParts) {
      for (s in 1:nSes) {
        for (c in 1:nCond) {
   ...
   /* Calculating the intitial reward probability */
   p_push = p_push_init;
   ...
   for (i in 1:N) {
       ...
       /*  Relative contribution of Action Value Learning verus Color Value Learning for Expected Values as a linear combination  */
       ...
       /* RL rule update for Action Value Learning */
       if (pushed[i] == 1){
