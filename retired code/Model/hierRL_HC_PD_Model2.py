...
pickelDir = subMainDirec + 'Model_secondOrder/hier/groups/hierRL_HC_PD_Model2.pkl'
if modelFit == True: 
    ...
    for c in range(0, n_chains):
        chaininit = {
            'z_alphaAct': np.random.uniform(-1, 1, size=(nParts, nGrps)),
            'z_alphClr': np.random.uniform(-1, 1, size=(nParts, nGrps)),        
            'z_weightAct': np.random.uniform(-1, 1, size=(nParts, nGrps)),
            'z_sensitivity': np.random.uniform(-1, 1, size=(nParts, nGrps)),
            'hier_alphaAct_sd': np.random.uniform(.01, .1, size=nGrps),
            'hier_alphaClr_sd': np.random.uniform(.01, .1, size=nGrps),        
            'hier_weightAct_sd': np.random.uniform(.01, .1, size=nGrps),
            'hier_sensitivity_sd': np.random.uniform(.01, .1, size=nGrps),
        }
        initials.append(chaininit)   

    # Loading the RL Stan Model
    file_name = '../stan_models/hier/hierRL_HC_PD_Model2.stan' 
...
fig.savefig(subMainDirec + 'Model_secondOrder/hier/groups/hierRL_HC_PD_Model2.png', dpi=300)
