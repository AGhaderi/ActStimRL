...
data {
    ...
    int<lower=1> nRuns;    // Number of Runs
    ...
    int<lower=1, upper=2> condition[N];        // Indices of cindition for each trials, 1: Action, 2: Color
    int<lower=1, upper=2> run[N];              // Indices of runs for each trials, 1: run 1, 2: run 2
...
parameters {
    matrix[nSes, nRuns, nCond] alphaAct;   // Learning rate for Action Learning Value
    matrix[nSes, nRuns, nCond] alphaClr;   // Learning rate for Color Learning Value
    matrix[nSes, nRuns, nCond] weightAct;  // Wieghtening of Action Learning Value against to Color Learnig Value
...
transformed parameters {
   ...
   for (i in 1:N) {
       ...
       EV_push_yell = weightAct[session[i], run[i], condition[i]]*EV_push + (1 - weightAct[session[i], run[i], condition[i]])*EV_yell;
       EV_push_blue = weightAct[session[i], run[i], condition[i]]*EV_push + (1 - weightAct[session[i], run[i], condition[i]])*EV_blue;
       EV_pull_yell = weightAct[session[i], run[i], condition[i]]*EV_pull + (1 - weightAct[session[i], run[i], condition[i]])*EV_yell;
       EV_pull_blue = weightAct[session[i], run[i], condition[i]]*EV_pull + (1 - weightAct[session[i], run[i], condition[i]])*EV_blue;
