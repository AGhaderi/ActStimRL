...
pickelDir = mainScarch  + 'realdata/hier/session/HierRL_' + str(cond_act_stim)+'_session_same_lr_model2.pkl'
if modelFit == True: 
    ...
    dataStan = {'N':behAll.shape[0],  
                ...
                'nGrps':nSess,
                'group':np.array(behAll.session).astype(int),
                'p_push_init':.5, 
                'p_yell_init':.5}
    # initial sampling
    initials = [] 
    for c in range(0, n_chains):
        chaininit = {
            'z_alpha': np.random.uniform(-1, 1, size=(nParts, nSess)),
            'z_weightAct': np.random.uniform(-1, 1, size=(nParts)),
            'z_sensitivity': np.random.uniform(-1, 1, size=(nParts, nSess)),
    ...
    file_name = '/mrhome/amingk/Documents/7TPD/ActStimRL/Model/stan_models/hier/HierRL_same_lr_model2.stan' 
    ...
    # Save Model Fit
    if not os.path.isdir(mainScarch  + 'realdata/hier/session'):
            os.makedirs(mainScarch  + 'realdata/hier/session') 
    # Save Model Fit
...
fig.savefig(mainScarch  + 'realdata/hier/session/HierRL_' + str(cond_act_stim)+'_session_same_lr_model2.png', dpi=300)
