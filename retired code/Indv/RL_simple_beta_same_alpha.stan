/* Model RL, two learning rate for action value learning and color value leanring*/ 
data {
...
parameters {
    array[nConds] real<lower=0, upper=1>  alphaAct;     // Learning rate for Action Learning Value
    array[nConds] real<lower=0, upper=1>  alphaClr;     // Learning rate for Color Learning Value
...
transformed parameters {
   ...
   for (i in 1:N) {
        ...
        if ((pushed[i] == 1 && yellowChosen[i] == 1) || (pushed[i] == 0 && yellowChosen[i] == 0))
            soft_max_EV[i] = exp(sensitivity*EV_push_yell)/(exp(sensitivity[condition[i]]*EV_push_yell) + exp(sensitivity[condition[i]]*EV_pull_blue));
        ...
        if (pushed[i] == 1){
            p_push = p_push + alphaAct[condition[i]]*(rewarded[i] - p_push); 
            p_pull = 1 - p_push;
        }
        else{
            p_pull = p_pull + alphaAct[condition[i]]*(rewarded[i] - p_pull);
            p_push = 1 - p_pull;
        }    
        if (yellowChosen[i] == 1){
           p_yell = p_yell + alphaClr[condition[i]]*(rewarded[i] - p_yell);
           p_blue = 1 - p_yell;
        }    
        else{
           p_blue = p_blue + alphaClr[condition[i]]*(rewarded[i] - p_blue);
           p_yell = 1 - p_blue;           
        }
