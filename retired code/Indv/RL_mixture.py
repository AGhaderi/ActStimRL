...
for session in [1, 2]: # session
    for condition in ['Act', 'Clr']: # condition
        behAll_sub = behAll[(behAll['sub_ID']==subName)&(behAll['block']==condition)&(behAll['session']==session)]  
        # The adrees name of pickle file
        pickelDir = subMainDirec + subName + '/model/indv/' + subName + '_cond-' + str(condition) + '_sess-' + str(session) +  '_RL_mixture.pkl'
        if modelFit == True: 
            ...
            for c in range(0, n_chains):
                chaininit = {
                    'alphaAct': RNG.uniform(.1, .5),
                    'alphClr': RNG.uniform(.1, .5),        
                    'sensitivity': RNG.uniform(.1, .5)
                }
                initials.append(chaininit)   

            # Loading the RL Stan Model
            file_name = '/home/amingk/Documents/7TPD/ActStimRL/Model/stan_models/indv/RL_mixture.stan' 
        ...
        fig.savefig(subMainDirec + subName + '/model/indv/' + subName + '_cond-' + str(condition) + '_sess-' + str(session) +  '_RL_mixure.png', dpi=300)
