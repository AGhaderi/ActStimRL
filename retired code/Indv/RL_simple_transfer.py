...
for session in [1, 2]: # session
    for reverse in [21, 14]: # two distinct environemnt
        for condition in ['Act', 'Clr']: # condition
            ...
            pickelDir = subMainDirec + subName + '/model/indv/' + subName + '_cond-' + str(condition) + '_sess-' + str(session) + '_env-' + str(reverse) + '_StanRL_simple_transfer.pkl'
            if modelFit == True: 
                ...
                file_name = '../stan_models/RL_simple_transfer.stan' 
            ...
            alphaAct_ = fit["transf_alphaAct"].flatten()
            alphaClr_ = fit["transf_alphaClr"].flatten() 
            weightAct_ = fit["transf_weightAct"].flatten()
            beta_ = fit["transf_sensitivity"].flatten() 
            ...
            fig.savefig(subMainDirec + subName + '/model/indv/' + subName + '_cond-' + str(condition) + '_sess-' + str(session) + '_env-' + str(reverse) + '_StanRL_simple_transfer.png', dpi=300)
