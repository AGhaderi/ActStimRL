...
transformed parameters {
   ...
   for (i in 1:N) {
        ...
        // soft max function
        sm_act[i] = exp(transfer_sensitivity[participant[i], condition[i]]*EV_push)/(exp(transfer_sensitivity[participant[i], condition[i]]*EV_push) + exp(transfer_sensitivity[participant[i], condition[i]]*EV_pull));
        ...
        // soft max function 
        sm_clr[i] = exp(transfer_sensitivity[participant[i], condition[i]]*EV_yell)/(exp(transfer_sensitivity[participant[i], condition[i]]*EV_yell) + exp(transfer_sensitivity[participant[i], condition[i]]*EV_blue));  
...
model { 
    ...
    for (i in 1:N) { 
        ...
        target += log_sum_exp(log(transfer_weightAct[participant[i], condition[i]]) + binomial_lpmf(pushed[i] | 1, sm_act[i]),
                              log1m(transfer_weightAct[participant[i], condition[i]]) + binomial_lpmf(yellowChosen[i] | 1, sm_clr[i]));
    }
}
generated quantities { 
   vector[N] log_lik;  
    ...
    for (i in 1:N) {
        log_lik[i] = log_sum_exp(log(transfer_weightAct[participant[i], condition[i]]) + binomial_lpmf(pushed[i] | 1, sm_act[i]),
                              log1m(transfer_weightAct[participant[i], condition[i]]) + binomial_lpmf(yellowChosen[i] | 1, sm_clr[i]));
        }
}