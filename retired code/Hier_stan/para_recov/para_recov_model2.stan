...
parameters {
    ...
    array[nConds] real hier_alphaAct_mu;    // Mean Hierarchical Learning rate for Action Learning Value
    array[nConds] real hier_alphaClr_mu;    // Mean Hierarchical Learning rate for Color Learning Value
    ...
    array[nParts, nConds] real z_alphaAct;   // Learning rate for Action Learning Value
    array[nParts, nConds] real z_alphaClr;   // Learning rate for Color Learning Value
...
transformed parameters {
    ...
    array[nParts, nConds] real<lower=0, upper=1> transfer_alphaAct;   // Learning rate for Action Learning Value
    array[nParts, nConds] real<lower=0, upper=1> transfer_alphaClr;   // Learning rate for Color Learning Value
    ...
    array[nConds] real<lower=0, upper=1> transfer_hier_alphaAct_mu;   // Hierarchical Learning rate for Action Learning Value
    array[nConds] real<lower=0, upper=1> transfer_hier_alphaClr_mu;   // Hierarchical Learning rate for Color Learning Value
    ...
    for (p in 1:nParts) {
        for (c in 1:nConds){
            transfer_alphaAct[p, c] = Phi(hier_alphaAct_mu[c] + z_alphaAct[p, c]*hier_alphaAct_sd);
            transfer_alphaClr[p, c] = Phi(hier_alphaClr_mu[c] + z_alphaClr[p, c]*hier_alphaClr_sd);
            transfer_weightAct[p, c] = Phi(hier_weightAct_mu[c] + z_weightAct[p, c]*hier_weightAct_sd);
   ...
   for (i in 1:N) {
        ...
        // Color value condition
            ...
            // yellow and pushed vs blue vs pulled 
            if ((pushed[i] == 1 && yellowChosen[i] == 1) || (pushed[i] == 0 && yellowChosen[i] == 0))
                soft_max_EV[i] = exp(transfer_sensitivity[participant[i], condition[i]]*EV_push_yell)/(exp(transfer_sensitivity[participant[i], condition[i]]*EV_push_yell) + exp(transfer_sensitivity[participant[i], condition[i]]*EV_pull_blue));

            // pushed and blue vs pulled and yellow
            if ((pushed[i] == 1 && yellowChosen[i] == 0) || (pushed[i] == 0 && yellowChosen[i] == 1))
                soft_max_EV[i] = exp(transfer_sensitivity[participant[i], condition[i]]*EV_pull_yell)/(exp(transfer_sensitivity[participant[i], condition[i]]*EV_pull_yell) + exp(transfer_sensitivity[participant[i], condition[i]]*EV_push_blue));  
        }
        ...
        if (pushed[i] == 1){
            p_push = p_push + transfer_alphaAct[participant[i], condition[i]]*(rewarded[i] - p_push); 
            p_pull = 1 - p_push;
        }
        else{
            p_pull = p_pull + transfer_alphaAct[participant[i], condition[i]]*(rewarded[i] - p_pull);
            p_push = 1 - p_pull;
        }    
        if (yellowChosen[i] == 1){
           p_yell = p_yell + transfer_alphaClr[participant[i], condition[i]]*(rewarded[i] - p_yell);
           p_blue = 1 - p_yell;
        }    
        else{
           p_blue = p_blue + transfer_alphaClr[participant[i], condition[i]]*(rewarded[i] - p_blue);
           p_yell = 1 - p_blue;           
        }
    }   
}
model { 
    /* Hierarchical mu parameter*/    
    for (c in 1:nConds){
        hier_alphaAct_mu[c] ~ normal(0,1);
        hier_alphaClr_mu[c] ~ normal(0,1); 
        hier_sensitivity_mu[c] ~ normal(0,1); 
        hier_weightAct_mu[c] ~ normal(0,1);
    }
    ...
    for (p in 1:nParts) {
        for (c in 1:nConds){
            z_alphaAct[p, c] ~ normal(0,1); 
            z_alphaClr[p, c] ~ normal(0,1);
            z_weightAct[p, c] ~ normal(0,1);
