...
parameters {
    ...
    array[nMeds, nConds, nEnvs] real hier_alphaAct_mu;    // Mean Hierarchical Learning rate for Action Learning Value
    array[nMeds, nConds, nEnvs] real hier_alphaClr_mu;    // Mean Hierarchical Learning rate for Color Learning Value
    ...
    array[nParts, nMeds, nConds, nEnvs] real z_alphaAct;   // Learning rate for Action Learning Value
    array[nParts, nMeds, nConds, nEnvs] real z_alphaClr;   // Learning rate for Color Learning Value
...
transformed parameters {
    ...
    array[nParts, nMeds, nConds, nEnvs] real<lower=0, upper=1> transfer_alphaAct;   // Learning rate for Action Learning Value
    array[nParts, nMeds, nConds, nEnvs] real<lower=0, upper=1> transfer_alphaClr;   // Learning rate for Color Learning Value
    ...
    array[nMeds, nConds, nEnvs] real<lower=0, upper=1> transfer_hier_alphaAct_mu;   // Hierarchical Learning rate for Action Learning Value
    array[nMeds, nConds, nEnvs] real<lower=0, upper=1> transfer_hier_alphaClr_mu;   // Hierarchical Learning rate for Color Learning Value
    ...
    for (p in 1:nParts) {
        for (m in 1:nMeds){
            for (c in 1:nConds){
                for (b in 1:nEnvs){
                    transfer_alphaAct[p, m, c, b] = Phi(hier_alphaAct_mu[m, c, b] + z_alphaAct[p, m, c, b]*hier_alphaAct_sd);
                    transfer_alphaClr[p, m, c, b] = Phi(hier_alphaClr_mu[m, c, b] + z_alphaClr[p, m, c, b]*hier_alphaClr_sd);
                }
   ...
   for (i in 1:N) {
        ...
        if (pushed[i] == 1){
            p_push = p_push + transfer_alphaAct[participant[i], medication[i], condition[i], environment[i]]*(rewarded[i] - p_push); 
            p_pull = 1 - p_push;
        }
        else{
            p_pull = p_pull + transfer_alphaAct[participant[i], medication[i], condition[i], environment[i]]*(rewarded[i] - p_pull);
            p_push = 1 - p_pull;
        }    
        if (yellowChosen[i] == 1){
           p_yell = p_yell + transfer_alphaClr[participant[i], medication[i], condition[i], environment[i]]*(rewarded[i] - p_yell);
           p_blue = 1 - p_yell;
        }    
        else{
           p_blue = p_blue + transfer_alphaClr[participant[i], medication[i], condition[i], environment[i]]*(rewarded[i] - p_blue);
           p_yell = 1 - p_blue;           
        }
    }   
}
model { 
    ...
    for (m in 1:nMeds){
        for (c in 1:nConds){
            for (b in 1:nEnvs){
                hier_alphaAct_mu[m, c, b] ~ normal(0,1);
                hier_alphaClr_mu[m, c, b] ~ normal(0,1); 
            }
            hier_sensitivity_mu[m, c] ~ normal(1,2); 
            hier_weightAct_mu[m, c] ~ normal(.5,.5);
        }
    } 
    ...
    for (p in 1:nParts) {
        for (m in 1:nMeds){
            for (c in 1:nConds){
                for (b in 1:nEnvs){
                    z_alphaAct[p, m, c, b] ~ normal(0, 1); 
                    z_alphaClr[p, m, c, b] ~ normal(0, 1);
                }
