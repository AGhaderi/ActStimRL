...
pickelDir = f'{mainScarch}/realdata/hier/PD/session/{model_name}_{cond_act_stim}.pkl'
...
if modelFit == True: 
    ...
    for c in range(0, n_chains):
        chaininit = {
            'z_alpha': np.random.uniform(-1, 1, size=(nParts, nSes)),
            'z_weightAct': np.random.uniform(-1, 1, size=(nParts)),
            'z_sensitivity': np.random.uniform(-1, 1, size=(nParts, nSes)),
    ...
    # Save Model Fit
    if not os.path.isdir(f'{mainScarch}/realdata/hier/PD/session/'):
            os.makedirs(f'{mainScarch}/realdata/hier/PD/session/') 
    # Save Model Fit
...
fig.savefig(f'{mainScarch}/realdata/hier/PD/session/{model_name}_{cond_act_stim}.png', dpi=300)
