...
model { 
    ...
    real soft_max_EV;  //  The soft-max function for each trial, trial-by-trial probability
    ...
    for (i in 1:N) {
        ...
        if ((pushed[i] == 1 && yellowChosen[i] == 1) || (pushed[i] == 0 && yellowChosen[i] == 0)){
            soft_max_EV = exp(transfer_sensitivity[participant[i], medication_session[i], condition[i]]*EV_push_yell)/(exp(transfer_sensitivity[participant[i], medication_session[i], condition[i]]*EV_push_yell) + exp(transfer_sensitivity[participant[i], medication_session[i], condition[i]]*EV_pull_blue));
            pushed[i] ~ bernoulli(soft_max_EV);
        }

        //  pushed/blue coded 1 and pulled/yellow coded 0
        else if ((pushed[i] == 1 && yellowChosen[i] == 0) || (pushed[i] == 0 && yellowChosen[i] == 1)){
            soft_max_EV = exp(transfer_sensitivity[participant[i], medication_session[i], condition[i]]*EV_push_blue)/(exp(transfer_sensitivity[participant[i], medication_session[i], condition[i]]*EV_push_blue) + exp(transfer_sensitivity[participant[i], medication_session[i], condition[i]]*EV_pull_yell));  
            pushed[i] ~ bernoulli(soft_max_EV);
        }
