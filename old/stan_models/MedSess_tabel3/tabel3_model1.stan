...
parameters {
    ...
    array[nConds, nMeds_nSes] real hier_alpha_neg_mu;    /EV_all/ Mean Hierarchical Negative Learning rate 
...
model { 
    ...
    /* RL likelihood */
    for (i in 1:N) { 
        pushed[i] ~ bernoulli_logit(soft_max_EV[i]);
        }
}
generated quantities { 
   vector[N] log_lik;  
    ...
    for (i in 1:N) {
        log_lik[i] = bernoulli_logit_lpmf(pushed[i] | soft_max_EV[i]);
    }
}