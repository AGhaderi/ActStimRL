...
def genertae_task_act_stim(isActFirst = True):
    ...
    session = np.concatenate([np.repeat(1, 168), # session 1
                              np.repeat(2, 168)]) # session 2

    # Four runs, each run contains two blocks (conditions)
    run = np.concatenate([np.repeat(1, 84), #The session 1 and run 1
                          np.repeat(2, 84), #The session 1 and run 2
                          np.repeat(1, 84), #The session 2 and run 1
                          np.repeat(2, 84)]) #The session 2 and run 2

    # the number of trials for eigh conditions, each condition (Action value or Stimulus Value) includes 42 trials
    trialNumber = np.repeat(np.repeat(1, 43), 8)
    
    # counterbalanced ledft and right stimulus response            
    yellowOnLeftSide = np.randon.rand()  
