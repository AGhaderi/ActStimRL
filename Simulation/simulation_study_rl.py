...
for n in np.linspace(0, 1, 11):
    ...
    rawBehAll_wonAmount_agent4 = rawBehAll.groupby(['group', 'block', 'sub_ID'], as_index=False)['wonAmount_'+ str(n)].sum()
    rawBehAll_wonAmount_agent4.loc[rawBehAll_wonAmount_agent4['group']==2,'wonAmount_'+ str(n)] = rawBehAll_wonAmount_agent4[rawBehAll_wonAmount_agent4['group']==2]['wonAmount_'+ str(n)]/2
    sns.barplot(data = rawBehAll_wonAmount_agent4, x='group', y='wonAmount_'+ str(n), hue='block', width=.5, errorbar="se", palette='Reds')
