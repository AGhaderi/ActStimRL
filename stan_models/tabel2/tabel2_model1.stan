...
parameters {
    ...
    array[nConds] real hier_alpha_pos_mu;       // Mean Hierarchical Positive Learning rate (unconstrained)
    array[nConds] real hier_alpha_neg_mu;       // Mean Hierarchical Negative Learning rate (unconstrained) 
    array[nConds] real hier_weight_mu;          // Mean Hierarchical Weighting (unconstrained) 
    array[nConds] real hier_sensitivity_mu;     // Mean Hierarchical snesitivity (unconstrained)
    ...
    array[nParts, nConds] real z_alpha_pos;     // Individual Positive Learning rate (unconstrained)
    array[nParts, nConds] real z_alpha_neg;     // Individual Negative Learning rate (unconstrained)
    array[nParts, nConds] real z_weight;        // Individual Wieghtening (unconstrained)
    array[nParts, nConds] real z_sensitivity;   // Individual Sensitivity (unconstrained)  
...
transformed parameters {
    ...
    array[nParts, nConds] real<lower=0, upper=1> transfer_alpha_pos;   // Individual Poistive Learning rate (constrained)  
    array[nParts, nConds] real<lower=0, upper=1> transfer_alpha_neg;   // Individual Negative Learning rate (constrained)  
    array[nParts, nConds] real<lower=0, upper=1> transfer_weight;      // Individual Wieghting (constrained)  
    array[nParts, nConds] real<lower=0> transfer_sensitivity;          // Individual Sensitivity (constrained) 
    
    /* Transfer Hierarchical parameters just for output*/
    array[nConds] real<lower=0, upper=1> transfer_hier_alpha_pos_mu;   // Hierarchical Positive Learning rate (constrained)
    array[nConds] real<lower=0, upper=1> transfer_hier_alpha_neg_mu;   // Hierarchical Negative Learning rate (constrained)
    array[nConds] real<lower=0, upper=1> transfer_hier_weight_mu;      // Hierarchical Wieghtening (constrained)
    array[nConds] real<lower=0> transfer_hier_sensitivity_mu;          // Hierarchical snesitivity (constrained)
    ...
    for (c in 1:nConds){
        transfer_hier_sensitivity_mu[c] = log1p_exp(hier_sensitivity_mu[c]);
    }
    ...
    for (p in 1:nParts) {
        for (c in 1:nConds){
            ...
            transfer_alpha_neg[p,c] = inv_logit(hier_alpha_neg_mu[c] + z_alpha_neg[p,c]*hier_alpha_sd);
            transfer_sensitivity[p,c] = log1p_exp(hier_sensitivity_mu[c] + z_sensitivity[p,c]*hier_sensitivity_sd);
        }
    }
   ...
   for (i in 1:N) {
        ...
        if ((pushed[i] == 1 && yellowChosen[i] == 1) || (pushed[i] == 0 && yellowChosen[i] == 0))
            EV_diff[i] =  transfer_sensitivity[participant[i], condition[i]] * (EV_push_yell - EV_pull_blue);
            
        // pushed and blue vs pulled and yellow
        if ((pushed[i] == 1 && yellowChosen[i] == 0) || (pushed[i] == 0 && yellowChosen[i] == 1))
            EV_diff[i] = transfer_sensitivity[participant[i], condition[i]] * (EV_push_blue - EV_pull_yell);
        ...
        if (pushed[i] == 1){
            ...
            // negative RPE
                ...
                p_push = p_push + transfer_alpha_neg[participant[i], condition[i]]*(rewarded[i] - p_push); 
            }
        }
        else{
            // positive RPE
            ...
            // negative RPE
                ...
                p_push = p_push - transfer_alpha_neg[participant[i], condition[i]]*(rewarded[i] + p_push - 1);
            }
        }   
        ...
        if (yellowChosen[i] == 1){
            ...
            // negative RPE
                ...
                p_yell = p_yell + transfer_alpha_neg[participant[i], condition[i]]*(rewarded[i] - p_yell);
            }
        }    
        else{
            // positive RPE
            ...
            // negative RPE
                ...
                p_yell = p_yell - transfer_alpha_neg[participant[i], condition[i]]*(rewarded[i] + p_yell - 1);
            }
        }
    }   
}
model { 
    ...
    for (c in 1:nConds){
        ...
        hier_alpha_neg_mu[c] ~ normal(0,2);
