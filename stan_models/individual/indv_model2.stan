...
parameters {
    ...
    array[nParts] real alpha_pos;    // Positive Learning rate
    array[nParts] real alpha_neg;    // Negative Learning rate
    array[nParts, nConds, nMeds_nSes] real weight;       // Wieghtening
    array[nParts] real sensitivity;  // Sensitivity  
}
transformed parameters {
    ...
    for (p in 1:nParts) {
        transfer_alpha_pos[p] = inv_logit(alpha_pos[p]);
        transfer_alpha_neg[p] = inv_logit(alpha_neg[p]);
        transfer_sensitivity[p] = log1p_exp(sensitivity[p]);
        for (c in 1:nConds){
...
model {       
    /* participant-level main paameter*/
    for (p in 1:nParts) {
        alpha_pos[p] ~ normal(0,3);
        alpha_neg[p] ~ normal(0,3);
        sensitivity[p] ~ normal(0,5); 
        for (c in 1:nConds){
            for (s in 1:nMeds_nSes){
