   ...
   for (i in 1:N) {
        ...
        if (pushed[i] == 1){
            ...
            if((rewarded[i] - p_push)>=0 ){ 
                p_push = p_push + transfer_alpha_pos[participant[i], medication_session[i]]*(rewarded[i] - p_push);
            } 
            // negative RPE
            else{
            ...
            if((rewarded[i] + p_push - 1)>=0){ 
                p_push = p_push - transfer_alpha_pos[participant[i], medication_session[i]]*(rewarded[i] + p_push - 1);
            } 
            // negative RPE
            else{
