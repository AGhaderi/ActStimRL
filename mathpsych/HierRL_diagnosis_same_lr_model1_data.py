...
behAll = behAll[behAll['block']==cond_act_stim]
...
pickelDir = f'{mainScarch}/realdata/hier/PD-HC/diagnosis/{model_name}_{cond_act_stim}.pkl'
if modelFit == True: 
    ...
    for c in range(0, n_chains):
        chaininit = {
            'z_alpha': np.random.uniform(-1, 1, size=(nParts, nGrps)),
            'z_weightAct': np.random.uniform(-1, 1, size=(nParts, nGrps)),
            'z_sensitivity': np.random.uniform(-1, 1, size=(nParts, nGrps)),
...
fig.savefig(f'figures/{model_name}_{cond_act_stim}.png', dpi=300)
...
plots.plot_posterior(x=weightAct_[0] - weightAct_[1], xlabel=r'$w_{(A)}$', ylabel='Density')
plt.title('Difference in Weighting parameter', fontsize=12)
plt.axvline(0, linestyle = '--')
print('Proportion w: ', np.mean((weightAct_[0] - weightAct_[1])>0))

# Sensitivity
fig.add_subplot(rows, columns, 2)
plots.plot_posterior(x=beta_[0]- beta_[1], xlabel=r'$\beta$', ylabel='Density')
plt.title('Difference in Sensitivity', fontsize=12)
plt.ylabel('Density', fontsize=12)
plt.xlabel(r'$\beta$', fontsize=14)
plt.axvline(0, linestyle = '--')
print('Proportion b: ', np.mean((beta_[0] - beta_[1])>0))

# Action Learning Rate
fig.add_subplot(rows, columns, 3)
plots.plot_posterior(x=alpha_[0]- alpha_[1], xlabel=r'$ \alpha $', ylabel='Density')
plt.title('Difference in Learning Rate', fontsize=12)
plt.axvline(0, linestyle = '--')
print('Proportion a: ', np.mean((alpha_[0] - alpha_[1])>0))

# Save figure of parameter distribution 
fig.savefig(f'figures/{model_name}_{cond_act_stim}_differnce.png', dpi=300)
