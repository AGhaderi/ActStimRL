...
behAll.block = behAll.block.replace(['Act', 'Stim'], [1, 2]).infer_objects(copy=False)
# number of participant
nParts = len(np.unique(behAll.sub_ID))
# participant indeces
behAll.sub_ID = behAll.sub_ID.replace(np.unique(behAll.sub_ID), np.arange(1, nParts +1)).infer_objects(copy=False)
...
if modelFit == True: 
    ...
    for c in range(0, n_chains):
        chaininit = {
            'hier_alphaAct_pos_mu1': .5*np.ones([nSes, nConds]),
            'hier_alphaAct_neg_mu1': .5*np.ones([nSes, nConds]),
            'hier_alphaClr_pos_mu1': .5*np.ones([nSes, nConds]),
            'hier_alphaClr_neg_mu1': .5*np.ones([nSes, nConds]),
            'hier_weight_mu1': .5*np.ones([nSes, nConds]),
            'hier_sensitivity_mu1': .05*np.ones([nSes, nConds]),
            'hier_alpha_sd1': .1,        
            'hier_weight_sd1': .1,
            'hier_sensitivity_sd1': .005,
            'z_alphaAct_pos1': .5*np.ones([nParts, nSes, nConds]),
            'z_alphaAct_neg1': .5*np.ones([nParts, nSes, nConds]),
            'z_alphaClr_pos1': .5*np.ones([nParts, nSes, nConds]),
            'z_alphaClr_neg1': .5*np.ones([nParts, nSes, nConds]),
            'z_weight1': .5*np.ones([nParts, nSes, nConds]),
            'z_sensitivity1': .05*np.ones([nParts, nSes, nConds]),
            'hier_alphaAct_mu2': .5*np.ones([nSes, nConds]),
            'hier_alphaClr_mu2': .5*np.ones([nSes, nConds]),
            'hier_weight_mu2': .5*np.ones([nSes, nConds]),
            'hier_sensitivity_mu2': .005*np.ones([nSes, nConds]),
            'hier_alpha_sd2': .1,        
            'hier_weight_sd2': .1,
            'hier_sensitivity_sd2': .005,
            'z_alphaAct2': .5*np.ones([nParts, nSes, nConds]),
            'z_alphaClr2': .5*np.ones([nParts, nSes, nConds]),
            'z_weight2': .5*np.ones([nParts, nSes, nConds]),
            'z_sensitivity2': .05*np.ones([nParts, nSes, nConds]),            
            'hier_theta_mu': .5*np.ones([nSes, nConds]),
            'hier_theta_sd': .1,
            'z_theta': .5*np.ones([nParts, nSes, nConds]),
        }
...
theta = fit["transfer_hier_theta_mu"]
...
sns.histplot(theta[0,0], kde=True, stat='density', bins=100)
sns.histplot(theta[0,1], kde=True, stat='density', bins=100)
sns.histplot(theta[1,0], kde=True, stat='density', bins=100)
sns.histplot(theta[1,1], kde=True, stat='density', bins=100)
