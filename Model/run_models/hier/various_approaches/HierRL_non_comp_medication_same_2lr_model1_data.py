...
pickelDir = f'{mainScarch}/realdata/hier/PD/various_approaches/{model_name}_{cond_act_stim}.pkl'
if modelFit == True: 
    ...
    for c in range(0, n_chains):
        chaininit = {
            'z_alpha_pos': np.random.uniform(-1, 1, size=(nParts, nMeds)),
            'z_alpha_neg': np.random.uniform(-1, 1, size=(nParts, nMeds)),
    ...
    # Save Model Fit
    if not os.path.isdir(mainScarch  + '/realdata/hier/PD/various_approaches/'):
            os.makedirs(mainScarch  + '/realdata/hier/PD/various_approaches/') 
    # Save Model Fit
...
alpha_pos = fit["transfer_hier_alpha_mu_pos"] 
alpha_neg = fit["transfer_hier_alpha_mu_neg"] 
...
sns.histplot(alpha_pos[0], kde=True, stat='density', bins=100)
sns.histplot(alpha_pos[1], kde=True, stat='density', bins=100)
plt.title('Positive Learning Rate', fontsize=12)
plt.ylabel('Density', fontsize=12)
...
sns.histplot(alpha_neg[0], kde=True, stat='density', bins=100)
sns.histplot(alpha_neg[1], kde=True, stat='density', bins=100)
plt.title('Negative Learning Rate', fontsize=12)
plt.ylabel('Density', fontsize=12)
plt.xlabel(r'$ \alpha $', fontsize=14)
plt.xlim(0, 1)
plt.legend(['PD OFF', 'PD ON']) 
...
fig.savefig(f'{mainScarch}/realdata/hier/PD/various_approaches/{model_name}_{cond_act_stim}.png', dpi=300)
