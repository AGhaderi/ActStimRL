...
behAll_group = behAll[behAll['group']==3]
...
for reverse in [21, 14]: # two distinct environemnt
    for condition in ['Act', 'Clr']: # condition
        behAll_cond = behAll_group[(behAll_group['block']==condition)&(behAll_group['reverse']==reverse)]  
        # The adrees name of pickle file
        pickelDir = subMainDirec + 'Model_secondOrder/hier/group3/' + 'cond-' + str(condition) + '_env-' + str(reverse) + '_Stan-hierRL_simple_transfer_group3.pkl'
        if modelFit == True: 
            ...
            for c in range(0, n_chains):
                chaininit = {
                    'z_alphaAct': np.random.normal(-1, 1, size=nParts),
                    'z_alphClr': np.random.normal(-1, 1, size=nParts),        
                    'z_weightAct': np.random.normal(-1, 1, size=nParts),
                    'z_sensitivity': np.random.normal(-1, .1, size=nParts),
                    'hier_alphaAct_sd': np.random.uniform(.01, .1),
                    'hier_alphClr_sd': np.random.uniform(.01, .1),        
            ...
            file_name = '../stan_models/hier/hierRL_simple_transfer.stan' 
        ...
        fig.savefig(subMainDirec + 'Model_secondOrder/hier/group3/' + 'cond-' + str(condition) + '_env-' + str(reverse) + '_Stan-hierRL_simple_transfer_group3.png', dpi=300)
