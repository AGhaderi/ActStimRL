...
simulated_data = simulated_data[(simulated_data['block']=='Act') & (simulated_data['reverse']==21)]
...
pickelDir = subMainDirec + 'Model_secondOrder/hier/para_recov/'+stan_file+'_sim_' + str(simNumber)+'.pkl'
if modelFit == True: 
    ...
    initials = [] 
    chaininit = {
        'alphaAct': np.random.uniform(.1, .3, size=(nParts)),
        'alphaClr': np.random.uniform(.1, .3, size=(nParts)),        
        'weightAct': np.random.uniform(.5, .5, size=(nParts)),
        'sensitivity': np.random.uniform(.01, .08, size=(nParts))
        }
    for c in range(0, n_chains):
    	initials.append(chaininit)  

    # Loading the RL Stan Model
    file_name = '/mrhome/amingk/Documents/7TPD/ActStimRL/Model/stan_models/hier/para_recov/'+stan_file+'.stan' 
...
hier_alphaAct_a = fit["hier_alphaAct_a"].flatten() 
hier_alphaAct_b = fit["hier_alphaAct_b"].flatten() 
hier_alphaClr_a = fit["hier_alphaClr_a"].flatten() 
hier_alphaClr_b = fit["hier_alphaClr_b"].flatten() 
hier_weightAct_a = fit["hier_weightAct_a"].flatten() 
hier_weightAct_b = fit["hier_weightAct_b"].flatten() 
beta_ = fit["hier_sensitivity_mu"].flatten()  
...
sns.histplot(hier_weightAct_dist, kde=True, stat='density')
...
sns.histplot(hier_alphaAct_dist, kde=True, stat='density')
...
sns.histplot(hier_alphaClr_dist, kde=True, stat='density')
...
fig.savefig(subMainDirec + 'Model_secondOrder/hier/para_recov/'+stan_file+'_sim_' + str(simNumber)+'.png', dpi=300)
