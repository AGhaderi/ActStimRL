...
"""Model fit for competing either Action Value Learning or Stimulus Value Learning in the cotext of Reinforcement Learning at the individual level"""
import numpy as np
import pandas as pd
import stan
...
for subName in subList_PD:
    ...
    pickelDir = mainScarch  + 'realdata/' + subName + '/model/indv/RL_' + str(cond_act_stim)+'_medication_same_lr_model1.pkl'
    ...
    if modelFit == True: 
        ...
        dataStan = {'N':int(behAll_sub.shape[0]),  
                    ...
                    'nGrps':nMeds,
                    'group':np.array(behAll_sub.group).astype(int),
                    'p_push_init':.5, 
                    'p_yell_init':.5}
        # initial sampling
        initials = [] 
        ...
        file_name = '/home/amingk/Documents/7TPD/ActStimRL/Model/stan_models/indv/RL_same_lr_model1.stan' 
    ...
    fig.savefig(mainScarch  + 'realdata/' + subName + '/model/indv/RL_' + str(cond_act_stim)+'_medication_same_lr_model1.png', dpi=300)
