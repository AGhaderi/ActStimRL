    ...
    for session in [1, 2]: # session
        for reverse in [21, 14]: # two distinct environemnt
            for condition in ['Act', 'Clr']: # condition
                ...
                # make /model/indv/ forlder for each participant
                if not os.path.exists(subMainDirec + subName + '/model/indv/'):
                    os.makedirs(subMainDirec + subName + '/model/indv/')
                # The adrees name of pickle file
                pickelDir = subMainDirec + subName + '/model/indv/' + subName + '_cond-' + str(condition) + '_sess-' + str(session) + '_env-' + str(reverse) + '_StanRL_simple_normal.pkl'
                if modelFit == True: 
                    ...
                    file_name = '../stan_models/indv/RL_simple_normal.stan' 
                ...
                fig.savefig(subMainDirec + subName + '/model/indv/' + subName + '_cond-' + str(condition) + '_sess-' + str(session) + '_env-' + str(reverse) + '_StanRL_simple_normal.png', dpi=300)
