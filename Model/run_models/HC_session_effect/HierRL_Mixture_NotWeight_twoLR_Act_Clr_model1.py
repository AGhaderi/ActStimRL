...
theta : Mixture contribution of Action Value Learning verus Color Value Learning
...
theta[2,2]  two session effect (OFF vs ON), two Conditions [Act, Clr]
...
if modelFit == True: 
    ...
    for c in range(0, n_chains):
        chaininit = {
            'z_alphaAct': np.random.uniform(-1, 1, size=(nParts, nSes, nConds)),
            'z_alphaClr': np.random.uniform(-1, 1, size=(nParts, nSes, nConds)),
            'z_sensitivity': np.random.uniform(-1, 1, size=(nParts, nSes, nConds)),
            'z_theta': np.random.uniform(-1, 1, size=(nParts, nSes, nConds)),
            'hier_alpha_sd': np.random.uniform(.01, .1),        
            'hier_sensitivity_sd': np.random.uniform(.01, .1),
            'hier_theta_sd': np.random.uniform(.01, .1),
        }
...
plt.title('Mixture parameter', fontsize=12)
