/*
Model 7 in Table 3. Checking out if latent paramters are changed by session in HC and medication in PD.
*/
...
transformed parameters {
    ...
    for (p in 1:nParts) {
        for (s in 1:nMeds_nSes){
            transfer_alphaAct_pos[p,s] = inv_logit(hier_alphaAct_pos_mu[s] + z_alphaAct_pos[p,s]*hier_alpha_sd); // here s referes to condition
