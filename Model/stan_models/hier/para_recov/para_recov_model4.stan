...
parameters {
    ...
    array[nConds] real<lower=0>  hier_alphaAct_a;    // Alpha Hierarchical Learning rate for Action Learning Value
    array[nConds] real<lower=0>  hier_alphaAct_b;    // Beta Hierarchical Learning rate for Action Learning Value
    array[nConds] real<lower=0>  hier_alphaClr_a;    // Alpha Hierarchical Learning rate for Color Learning Value
    array[nConds] real<lower=0>  hier_alphaClr_b;    // Beta Hierarchical Learning rate for Color Learning Value
    array[nConds] real<lower=0>  hier_weightAct_a;   // Alpha Hierarchical Wieghtening of Action Learning Value against to Color Learnig Value
    array[nConds] real<lower=0>  hier_weightAct_b;   // Beta Hierarchical Wieghtening of Action Learning Value against to Color Learnig Value
    array[nConds] real<lower=0> hier_sensitivity_mu;    // Mean Hierarchical snesitivity, With a higher sensitivity value θ, choices are more sensitive to value differences
    real<lower=0> hier_sensitivity_sd;    // Between-participant variability sensitivity

    /* participant-level main paameter*/
    array[nParts, nConds] real<lower=0, upper=1> alphaAct;   // Learning rate for Action Learning Value
    array[nParts, nConds] real<lower=0, upper=1> alphaClr;   // Learning rate for Color Learning Value
    array[nParts, nConds] real<lower=0, upper=1> weightAct;  // Wieghtening of Action Learning Value against to Color Learnig Value
    array[nParts, nConds] real<lower=0> sensitivity;         // With a higher sensitivity value θ, choices are more sensitive to value differences
...
transformed parameters {
   ...
   for (i in 1:N) {
        ...
        // soft max function
        sm_act[i] = exp(sensitivity[participant[i], condition[i]]*EV_push)/(exp(sensitivity[participant[i], condition[i]]*EV_push) + exp(sensitivity[participant[i], condition[i]]*EV_pull));
        ...
        // soft max function 
        sm_clr[i] = exp(sensitivity[participant[i], condition[i]]*EV_yell)/(exp(sensitivity[participant[i], condition[i]]*EV_yell) + exp(sensitivity[participant[i], condition[i]]*EV_blue));  
        ...
        if (pushed[i] == 1){
            p_push = p_push + alphaAct[participant[i], condition[i]]*(rewarded[i] - p_push); 
            p_pull = 1 - p_push;
        }
        else{
            p_pull = p_pull + alphaAct[participant[i], condition[i]]*(rewarded[i] - p_pull);
            p_push = 1 - p_pull;
        }    
        if (yellowChosen[i] == 1){
           p_yell = p_yell + alphaClr[participant[i], condition[i]]*(rewarded[i] - p_yell);
           p_blue = 1 - p_yell;
        }    
        else{
           p_blue = p_blue + alphaClr[participant[i], condition[i]]*(rewarded[i] - p_blue);
           p_yell = 1 - p_blue;           
        }
...
model { 
    /* Hierarchical mu parameter*/    
    for (c in 1:nConds){
        hier_alphaAct_a[c] ~ normal(0,3);
        hier_alphaAct_b[c] ~ normal(0,3); 
        hier_alphaClr_a[c] ~ normal(0,3); 
        hier_alphaClr_b[c] ~ normal(0,3);
        hier_weightAct_a[c] ~ normal(3,1); 
        hier_weightAct_b[c] ~ normal(3,1);
    ...
    for (p in 1:nParts) {
        for (c in 1:nConds){
            alphaAct[p, c] ~ beta(hier_alphaAct_a[c], hier_alphaAct_b[c]); 
            alphaClr[p, c] ~ beta(hier_alphaClr_a[c], hier_alphaClr_b[c]);
            weightAct[p, c] ~ beta(hier_weightAct_a[c], hier_weightAct_b[c]);
            sensitivity[p, c] ~ normal(hier_sensitivity_mu[c], hier_sensitivity_sd); 
            }   
    }
    ...
    for (i in 1:N) { 
        ...
        target += log_sum_exp(log(weightAct[participant[i], condition[i]]) + binomial_lpmf(pushed[i] | 1, sm_act[i]),
                              log1m(weightAct[participant[i], condition[i]]) + binomial_lpmf(yellowChosen[i] | 1, sm_clr[i]));
    }
}
generated quantities { 
   vector[N] log_lik;  
    ...
    for (i in 1:N) {
        log_lik[i] = log_sum_exp(log(weightAct[participant[i], condition[i]]) + binomial_lpmf(pushed[i] | 1, sm_act[i]),
                              log1m(weightAct[participant[i], condition[i]]) + binomial_lpmf(yellowChosen[i] | 1, sm_clr[i]));
        }
}