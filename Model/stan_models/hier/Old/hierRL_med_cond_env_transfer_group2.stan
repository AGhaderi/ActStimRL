...
data {
    ...
    int<lower=1> nEnvs;  // Number of environment, two diffeernt environemnt, stable and volatile environemnts
    array[N] int<lower=1, upper=2> condition;   // 1 indecates Action value learning and 2 indicates Color value learning
    array[N] int<lower=1, upper=2> environment; // different environemnt, 21 is coded 1 as stable environemnt, 14 is coded 2 as volatile environemnt
...
parameters {
    ...
    array[nConds, nEnvs] real hier_alphaAct_mu;    // Mean Hierarchical Learning rate for Action Learning Value
    array[nConds, nEnvs] real hier_alphaClr_mu;    // Mean Hierarchical Learning rate for Color Learning Value
    array[nConds] real hier_weightAct_mu;   // Mean Hierarchical Wieghtening of Action Learning Value against to Color Learnig Value
    array[nConds] real hier_sensitivity_mu;    // Mean Hierarchical snesitivity, With a higher sensitivity value θ, choices are more sensitive to value differences
    ...
    array[nParts, nConds, nEnvs] real z_alphaAct;   // Learning rate for Action Learning Value
    array[nParts, nConds, nEnvs] real z_alphaClr;   // Learning rate for Color Learning Value
    array[nParts, nConds] real z_weightAct;  // Wieghtening of Action Learning Value against to Color Learnig Value
    array[nParts, nConds] real z_sensitivity;         // With a higher sensitivity value θ, choices are more sensitive to value differences
...
transformed parameters {
    ...
    array[nParts, nConds, nEnvs] real<lower=0, upper=1> transfer_alphaAct;   // Learning rate for Action Learning Value
    array[nParts, nConds, nEnvs] real<lower=0, upper=1> transfer_alphaClr;   // Learning rate for Color Learning Value
    array[nParts, nConds] real<lower=0, upper=1> transfer_weightAct;  // Wieghtening of Action Learning Value against to Color Learnig Value
    array[nParts, nConds] real<lower=0> transfer_sensitivity;         // With a higher sensitivity value θ, choices are more sensitive to value differences
    
    /* Transfer Hierarchical parameters just for output*/
    array[nConds, nEnvs] real<lower=0, upper=1> transfer_hier_alphaAct_mu;   // Hierarchical Learning rate for Action Learning Value
    array[nConds, nEnvs] real<lower=0, upper=1> transfer_hier_alphaClr_mu;   // Hierarchical Learning rate for Color Learning Value
    array[nConds] real<lower=0, upper=1> transfer_hier_weightAct_mu;  // Hierarchical Wieghtening of Action Learning Value against to Color Learnig Value
    array[nConds] real<lower=0> transfer_hier_sensitivity_mu;         // Hierarchical snesitivity, With a higher sensitivity value θ, choices are more sensitive to value differences
    ...
    for (c in 1:nConds){
        transfer_hier_sensitivity_mu[m] = log(1 + exp(hier_sensitivity_mu[m]));
    }

    for (p in 1:nParts) {
        for (c in 1:nConds){
            for (b in 1:nEnvs){
                transfer_alphaAct[p, m, b] = Phi(hier_alphaAct_mu[m, b] + z_alphaAct[p, m, b]*hier_alphaAct_sd);
                transfer_alphaClr[p, m, b] = Phi(hier_alphaClr_mu[m, b] + z_alphaClr[p, m, b]*hier_alphaClr_sd);
            }
            transfer_weightAct[p, m] = Phi(hier_weightAct_mu[m] + z_weightAct[p, m]*hier_weightAct_sd);
            transfer_sensitivity[p, m] = log(1 + exp(hier_sensitivity_mu[m] + z_sensitivity[p, m]*hier_sensitivity_sd));
        }
    }
   ...
   for (i in 1:N) {
        ...
        EV_push_yell = transfer_weightAct[participant[i], condition[i]]*EV_push + (1 - transfer_weightAct[participant[i], condition[i]])*EV_yell;
        EV_push_blue = transfer_weightAct[participant[i], condition[i]]*EV_push + (1 - transfer_weightAct[participant[i], condition[i]])*EV_blue;
        EV_pull_yell = transfer_weightAct[participant[i], condition[i]]*EV_pull + (1 - transfer_weightAct[participant[i], condition[i]])*EV_yell;
        EV_pull_blue = transfer_weightAct[participant[i], condition[i]]*EV_pull + (1 - transfer_weightAct[participant[i], condition[i]])*EV_blue;
        ...
        if ((pushed[i] == 1 && yellowChosen[i] == 1) || (pushed[i] == 0 && yellowChosen[i] == 0))
            soft_max_EV[i] = exp(transfer_sensitivity[participant[i], condition[i]]*EV_push_yell)/(exp(transfer_sensitivity[participant[i], condition[i]]*EV_push_yell) + exp(transfer_sensitivity[participant[i], condition[i]]*EV_pull_blue));

        // pushed and blue vs pulled and yellow
        if ((pushed[i] == 1 && yellowChosen[i] == 0) || (pushed[i] == 0 && yellowChosen[i] == 1))
            soft_max_EV[i] = exp(transfer_sensitivity[participant[i], condition[i]]*EV_push_blue)/(exp(transfer_sensitivity[participant[i], condition[i]]*EV_push_blue) + exp(transfer_sensitivity[participant[i], condition[i]]*EV_pull_yell));  
        ...
        if (pushed[i] == 1){
            p_push = p_push + transfer_alphaAct[participant[i], condition[i], environment[i]]*(rewarded[i] - p_push); 
            p_pull = 1 - p_push;
        }
        else{
            p_pull = p_pull + transfer_alphaAct[participant[i], condition[i], environment[i]]*(rewarded[i] - p_pull);
            p_push = 1 - p_pull;
        }    
        if (yellowChosen[i] == 1){
           p_yell = p_yell + transfer_alphaClr[participant[i], condition[i], environment[i]]*(rewarded[i] - p_yell);
           p_blue = 1 - p_yell;
        }    
        else{
           p_blue = p_blue + transfer_alphaClr[participant[i], condition[i], environment[i]]*(rewarded[i] - p_blue);
           p_yell = 1 - p_blue;           
        }
    }   
}
model { 
    ...
    for (c in 1:nConds){
        for (b in 1:nEnvs){
            hier_alphaAct_mu[m, b] ~ normal(0,1);
            hier_alphaClr_mu[m, b] ~ normal(0,1); 
        }
        hier_sensitivity_mu[m] ~ normal(1,2); 
        hier_weightAct_mu[m] ~ normal(.5,.5);
    }
    ...
    for (p in 1:nParts) {
        for (c in 1:nConds){
            for (b in 1:nEnvs){
                z_alphaAct[p, m, b] ~ normal(0, 1); 
                z_alphaClr[p, m, b] ~ normal(0, 1);
            }
            z_weightAct[p, m] ~ normal(0, 1);
            z_sensitivity[p, m] ~ normal(0, 1); 
        }   
    }
