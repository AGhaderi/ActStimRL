...
model { 
    ...
    for (i in 1:N) { 
        vector[2] lps;
        lps[1] = log(transfer_theta[participant[i], medication_session[i], condition[i]]) + binomial_lpmf(pushed[i] | 1, soft_max_EV1[i]);  // first term of mixture 
        lps[2] = log1m(transfer_theta[participant[i], medication_session[i], condition[i]]) + binomial_lpmf(pushed[i] | 1, soft_max_EV2[i]);  // second term of mixture
