...
transformed parameters {
    ...
    for (p in 1:nParts) {
        for (g in 1:nGrps){
            for (c in 1:nConds){
                transfer_alphaAct[p, g, c] = Phi(hier_alphaAct_mu[c] + z_alphaAct[p, g, c]*hier_alphaAct_sd[group[i]]);
                transfer_alphaClr[p, g, c] = Phi(hier_alphaClr_mu[c] + z_alphaClr[p, g, c]*hier_alphaClr_sd[group[i]]);
                transfer_weightAct[p, g, c] = Phi(hier_weightAct_mu[c] + z_weightAct[p, g, c]*hier_weightAct_sd[group[i]]);
                transfer_sensitivity[p, g, c] = log(1 + exp(hier_sensitivity_mu[c] + z_sensitivity[p, g, c]*hier_sensitivity_sd[group[i]]));
            }
        }
    }
