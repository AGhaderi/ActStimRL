/*  Model 4 assumes that there is meaningful effect across latent parameters, enviroenment as well as group labels {HC and PD}.
...
transformed parameters {
    ...
    for (p in 1:nParts) {
        for (g in 1:nGrps){
            for (c in 1:nConds){
                transfer_alphaAct[p, g, c] = Phi(hier_alphaAct_mu[g, c] + z_alphaAct[p, g, c]*hier_alphaAct_sd[g]);
                transfer_alphaClr[p, g, c] = Phi(hier_alphaClr_mu[g, c] + z_alphaClr[p, g, c]*hier_alphaClr_sd[g]);
                transfer_weightAct[p, g, c] = Phi(hier_weightAct_mu[g, c] + z_weightAct[p, g, c]*hier_weightAct_sd[g]);
                transfer_sensitivity[p, g, c] = log(1 + exp(hier_sensitivity_mu[g, c] + z_sensitivity[p, g, c]*hier_sensitivity_sd[g]));
            }
        }
    }
