...
parameters {
    ...
    real hier_alpha_mu;    // Mean Hierarchical Learning rate for Learning Value
    ...
    array[nParts] real z_alpha;   // Learning rate for Learning Value
...
transformed parameters {
    ...
    array[nParts] real<lower=0, upper=1> transfer_alpha;   // Learning rate for Learning Value
    ...
    real<lower=0, upper=1> transfer_hier_alpha_mu;   // Hierarchical Learning rate for Action Learning Value
    ...
    for (p in 1:nParts) {
        ...
        transfer_alpha[p] = Phi(hier_alpha_mu + z_alpha[p]*hier_alpha_sd);
    }
   ...
   for (i in 1:N) {
        ...
        if (pushed[i] == 1){
            ...
            p_pull = p_pull + transfer_alpha[participant[i]]*(rewarded[i] - p_pull);
            p_push = 1 - p_pull;
        }    
        if (yellowChosen[i] == 1){
           ...
           p_blue = p_blue + transfer_alpha[participant[i]]*(rewarded[i] - p_blue);
           p_yell = 1 - p_blue;           
        }
    }   
}
