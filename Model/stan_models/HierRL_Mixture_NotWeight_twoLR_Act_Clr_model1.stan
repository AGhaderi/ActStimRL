...
transformed parameters {
   ...
   for (i in 1:N) {
       ...
       // change the probability of 1 to near to 1, to avoid further exception in Model block  
        if (soft_max_Act[i]==1){
            soft_max_Act[i] =  0.99999999999999;
        }
       ...
       // change the probability of 1 to near to 1, to avoid further exception in Model block  
        if (soft_max_Stim[i]==1){
            soft_max_Stim[i] =  0.99999999999999;
        }
...
model { 
    ...
    for (i in 1:N) { 
        ...
        vector[2] lps;
        lps[1] = log(transfer_theta[participant[i], medication_session[i], condition[i]]) + bernoulli_lpmf(pushed[i] | soft_max_Act[i]);  // first term of mixture 
        lps[2] = log1m(transfer_theta[participant[i], medication_session[i], condition[i]]) + bernoulli_lpmf(yellowChosen[i] | soft_max_Stim[i]);  // second term of mixture
        // target
