...
transformed parameters {
    ...
    for (p in 1:nParts) {
        for (s in 1:nMeds_nSes){
            transfer_weight[p,s] = inv_logit(hier_weight_mu[s] + z_weight[p,s]*hier_weight_sd); // here s refers to condition
...
model { 
    /* Hierarchical mu parameter*/  
    for (s in 1:nMeds_nSes){
        hier_weight_mu[s] ~ normal(0,2); // here s refers to condition
    ...
    for (p in 1:nParts) {
        for (s in 1:nMeds_nSes){
            z_weight[p,s] ~ normal(0,1);// here s refers to conditions
