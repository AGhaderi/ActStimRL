/*
Model 2 in Table 3. Checking out if latent paramters are changed by session in HC and medication in PD.
*/
...
parameters {
    ...
    array[nParts, nMeds_nSes] real z_sensitivity;    // Inidividual sensitivity 
...
transformed parameters {
    ...
    array[nParts, nMeds_nSes] real<lower=0> transfer_sensitivity;             // Transfered sensitivity 
    ...
    for (p in 1:nParts) {
        for (s in 1:nMeds_nSes){
            transfer_alphaAct_pos[p,s] = inv_logit(hier_alphaAct_pos_mu[s] + z_alphaAct_pos[p,s]*hier_alpha_sd); // here s refers to condition
            transfer_alphaAct_neg[p,s] = inv_logit(hier_alphaAct_neg_mu[s] + z_alphaAct_neg[p,s]*hier_alpha_sd);
            transfer_alphaClr_pos[p,s] = inv_logit(hier_alphaClr_pos_mu[s] + z_alphaClr_pos[p,s]*hier_alpha_sd);
            transfer_alphaClr_neg[p,s] = inv_logit(hier_alphaClr_neg_mu[s] + z_alphaClr_neg[p,s]*hier_alpha_sd);

            transfer_sensitivity[p,s] = log(1 + exp(hier_sensitivity_mu[s] + z_sensitivity[p,s]*hier_sensitivity_sd)); // here s refers to session
            for (c in 1:nConds){
...
model { 
    /* Hierarchical mu parameter*/  
    for (s in 1:nMeds_nSes){
        hier_alphaAct_pos_mu[s] ~ normal(0,2); // here s refers to condition
        hier_alphaAct_neg_mu[s] ~ normal(0,2);
        hier_alphaClr_pos_mu[s] ~ normal(0,2);
        hier_alphaClr_neg_mu[s] ~ normal(0,2);
    ...
    for (p in 1:nParts) {
        for (s in 1:nMeds_nSes){
            z_alphaAct_pos[p,s] ~ normal(0,1); // here s refers to condition
            z_alphaAct_neg[p,s] ~ normal(0,1);
            z_alphaClr_pos[p,s] ~ normal(0,1);
            z_alphaClr_neg[p,s] ~ normal(0,1);
