...
transformed parameters {
   ...
   for (i in 1:N) {
        ...
        if ((leftChosen[i] == 1 && yellowChosen[i] == 1) || (leftChosen[i] == 0 && yellowChosen[i] == 0))
            soft_max_EV[i] = exp(transfer_sensitivity[participant[i], medication_session[i]]*EV_left_yell)/(exp(transfer_sensitivity[participant[i], medication_session[i]]*EV_left_yell) + exp(transfer_sensitivity[participant[i], medication_session[i]]*EV_right_blue));

        //  left/blue coded 1 and right/yellow coded 0
        else if ((leftChosen[i] == 1 && yellowChosen[i] == 0) || (leftChosen[i] == 0 && yellowChosen[i] == 1))
            soft_max_EV[i] = exp(transfer_sensitivity[participant[i], medication_session[i]]*EV_left_blue)/(exp(transfer_sensitivity[participant[i], medication_session[i]]*EV_left_blue) + exp(transfer_sensitivity[participant[i], medication_session[i]]*EV_right_yell));  
