...
parameters {
    ...
    array[nConds] real hier_alphaAct_pos_mu;  // Mean Hierarchical Positive Learning rate for action Learning Value  
    array[nConds] real hier_alphaAct_neg_mu;  // Mean Hierarchical Negative Learning rate for action Learning Value  
    array[nConds] real hier_alphaClr_pos_mu;  // Mean Hierarchical Positive Learning rate for color Learning Value  
    array[nConds] real hier_alphaClr_neg_mu;  // Mean Hierarchical Negative Learning rate for color Learning Value  
    real hier_weight_mu;        // Mean Hierarchical Wieghting
    ...
    array[nParts, nConds] real z_alphaAct_pos;   // IndividualPositive Learning rate for Action Learning Value
    array[nParts, nConds] real z_alphaAct_neg;   // Individual Negative Learning rate for Action Learning Value
    array[nParts, nConds] real z_alphaClr_pos;   // Individual Positive Learning rate for Color Learning Value
    array[nParts, nConds] real z_alphaClr_neg;   // Individual Negative Learning rate for Color Learning Value
    array[nParts] real z_weight;         // Individual weighing
...
transformed parameters {
    ...
    array[nParts, nConds] real<lower=0, upper=1> transfer_alphaAct_pos;   // Transfered Poistive Learning rate for Action Learning Value
    array[nParts, nConds] real<lower=0, upper=1> transfer_alphaAct_neg;   // Transfered Negative Learning rate for Action Learning Value
    array[nParts, nConds] real<lower=0, upper=1> transfer_alphaClr_pos;   // Transfered  Positive Learning rate for Color Learning Value
    array[nParts, nConds] real<lower=0, upper=1> transfer_alphaClr_neg;   // Transfered  Negative Learning rate for Color Learning Value
    array[nParts] real<lower=0, upper=1> transfer_weight;         // Transfered  Wieghtening  
    ...
    array[nConds] real<lower=0, upper=1> transfer_hier_alphaClr_pos_mu;   // Hierarchical Positive Learning rate for Color Learning Value
    array[nConds] real<lower=0, upper=1> transfer_hier_alphaClr_neg_mu;   // Hierarchical Negative  Learning rate for Color Learning Value
    ...
    for (p in 1:nParts) {
        ...
        for (c in 1:nConds){
            transfer_alphaAct_pos[p,c] = inv_logit(hier_alphaAct_pos_mu[c] + z_alphaAct_pos[p,c]*hier_alpha_sd);
            transfer_alphaAct_neg[p,c] = inv_logit(hier_alphaAct_neg_mu[c] + z_alphaAct_neg[p,c]*hier_alpha_sd);
            transfer_alphaClr_pos[p,c] = inv_logit(hier_alphaClr_pos_mu[c] + z_alphaClr_pos[p,c]*hier_alpha_sd);
            transfer_alphaClr_neg[p,c] = inv_logit(hier_alphaClr_neg_mu[c] + z_alphaClr_neg[p,c]*hier_alpha_sd);
   ...
   for (i in 1:N) {
        ...
        if (pushed[i] == 1){
            ...
            if((rewarded[i] + p_push -1)>=0){ 
                p_push = p_push - transfer_alphaAct_pos[participant[i], condition[i]]*(rewarded[i] + p_push -1);
            } 
            // negative RPE
            else{
                p_push = p_push - transfer_alphaAct_neg[participant[i], condition[i]]*(rewarded[i] + p_push -1);
            }
        }   
        ...
        if (yellowChosen[i] == 1){
            ...
            if((rewarded[i] - p_yell)>=0){ 
                p_yell = p_yell + transfer_alphaClr_pos[participant[i], condition[i]]*(rewarded[i] - p_yell);
            } 
            // negative RPE
            else{
                p_yell = p_yell + transfer_alphaClr_neg[participant[i], condition[i]]*(rewarded[i] - p_yell);
            }
        }    
        else{
            // positive RPE
            if((rewarded[i] + p_yell -1)>=0){ 
                p_yell = p_yell - transfer_alphaClr_pos[participant[i], condition[i]]*(rewarded[i] + p_yell - 1);
            } 
            // negative RPE
            else{
                p_yell= p_yell - transfer_alphaClr_neg[participant[i], condition[i]]*(rewarded[i] + p_yell -1);
            }
        }
    }   
}
model { 
    ...
    for (c in 1:nConds){
        hier_alphaAct_pos_mu[c] ~ normal(0,2);
        hier_alphaAct_neg_mu[c] ~ normal(0,2);
        hier_alphaClr_pos_mu[c] ~ normal(0,2);
        hier_alphaClr_neg_mu[c] ~ normal(0,2);
    ...
    for (p in 1:nParts) {
        ...
        for (c in 1:nConds){
            ...
            z_alphaClr_pos[p,c] ~ normal(0,1);
            z_alphaClr_neg[p,c] ~ normal(0,1);
