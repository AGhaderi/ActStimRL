...
data {
    ...
    int<lower=1> nGrps;   // Number of groups, Action and Color value learning
    array[N] int<lower=1, upper=2> group;  // 1 indecates Action value learning and 2 indicates Color value learning
...
parameters {
    array[nGrps] real<lower=0, upper=1>  alpha;        // Learning rate
    array[nGrps] real<lower=0, upper=1>  weightAct;    // Wieghtening of Action Learning Value against to Color Learnig Value
    array[nGrps] real<lower=0>  sensitivity;           // With a higher sensitivity value θ, choices are more sensitive to value differences
}
transformed parameters {
   ...
   for (i in 1:N) {
        ...
        EV_push_yell = weightAct[group[i]]*EV_push + (1 - weightAct[group[i]])*EV_yell;
        EV_push_blue = weightAct[group[i]]*EV_push + (1 - weightAct[group[i]])*EV_blue;
        EV_pull_yell = weightAct[group[i]]*EV_pull + (1 - weightAct[group[i]])*EV_yell;
        EV_pull_blue = weightAct[group[i]]*EV_pull + (1 - weightAct[group[i]])*EV_blue;
        ...
        if ((pushed[i] == 1 && yellowChosen[i] == 1) || (pushed[i] == 0 && yellowChosen[i] == 0))
            soft_max_EV[i] = exp(sensitivity[group[i]]*EV_push_yell)/(exp(sensitivity[group[i]]*EV_push_yell) + exp(sensitivity[group[i]]*EV_pull_blue));

        // pushed and blue vs pulled and yellow
        if ((pushed[i] == 1 && yellowChosen[i] == 0) || (pushed[i] == 0 && yellowChosen[i] == 1))
            soft_max_EV[i] = exp(sensitivity[group[i]]*EV_push_blue)/(exp(sensitivity[group[i]]*EV_push_blue) + exp(sensitivity[group[i]]*EV_pull_yell));  
        ...
        if (pushed[i] == 1){
            p_push = p_push + alpha[group[i]]*(rewarded[i] - p_push); 
            p_pull = 1 - p_push;
        }
        else{
            p_pull = p_pull + alpha[group[i]]*(rewarded[i] - p_pull);
            p_push = 1 - p_pull;
        }    
        if (yellowChosen[i] == 1){
           p_yell = p_yell + alpha[group[i]]*(rewarded[i] - p_yell);
           p_blue = 1 - p_yell;
        }    
        else{
           p_blue = p_blue + alpha[group[i]]*(rewarded[i] - p_blue);
           p_yell = 1 - p_blue;           
        }
